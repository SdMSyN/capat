<?php
    session_start();
    include ('../config/conexion.php');
    $user = $_POST['inputUser'];
    $pass = $_POST['inputPass'];
    
    $cadErr = '';
    $ban =false;
    $perfil = 0;
    
    $sqlGetUser = "SELECT $tUsers.id as id, $tUsers.user as user, $tUPerfil.id as idPerfil, $tUPerfil.nombre as perfil "
            . "FROM $tUsers "
						. "INNER JOIN $tUPerfil ON $tUPerfil.id = $tUsers.perfil_id "
            . "WHERE BINARY $tUsers.user='$user' AND BINARY $tUsers.pass='$pass' ";
    $resGetUser=$con->query($sqlGetUser);
    if($resGetUser->num_rows > 0){
				if (array_key_exists('inputCheckRecuerdame',$_POST)) {
					// Crear un nuevo cookie de sesion, que expira a los 30 días
					ini_set('session.cookie_lifetime', 60 * 60 * 24 * 30);
					session_regenerate_id(TRUE);
				}
        $rowGetUser=$resGetUser->fetch_assoc();
        $_SESSION['sessU'] = true;
        $_SESSION['userId'] = $rowGetUser['id'];
        $_SESSION['userName'] = $rowGetUser['user'];
        $_SESSION['userPerfil'] = $rowGetUser['idPerfil'];
        $perfil = $rowGetUser['idPerfil'];
				$_SESSION['namePerfil'] = $rowGetUser['perfil'];
				$_SESSION['userLogo'] = 'avatar04.png';
        $ban = true;
    }
    else{ 
        $_SESSION['sessU']=false;
        $cadErr = "Usuario incorrecto";
        $ban = false;
    }
                        
    if($ban){
        echo json_encode(array("error"=>0, "perfil"=>$perfil));
    }else{
        echo json_encode(array("error"=>1, "msgErr"=>$cadErr, "sql"=>$sqlGetUser));
    }
?>