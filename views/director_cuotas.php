  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
			<div class="row">
				...
				<div class="col-xs-5 ">
					<div class="btn-group pull-right">
						<a href="#" class="btn btn-default" data-toggle="modal" data-target="#modalAddCuota"><i class="fa fa-plus"></i> Nuevo</a>
			...
			<!-- modal añadir servicio -->
				<form class="form-horizontal" id="formAddCuota" name="formAddCuota">
					<div class="modal fade" id="modalAddCuota" tabindex="-1" role="dialog" aria-labellebdy="myModalLabel">
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="hidden" id="inputIDColony" name="inputIDColony" value="<?= $idColony; ?>">
											<select class="form-control" id="inputYear" name="inputYear" required>
												<?php 
													$i = 1980;
													$cadOptionYear = '<option></option>';
													for($a = 2018; $a >= $i; $a--){
														$cadOptionYear .= '<option value="'.$a.'">'.$a.'</option>';
													}
													echo $cadOptionYear;
												?>
											</select>
									...
									<div class="form-group">
										<label for="inputCuota" class="col-sm-3 control-label">Cantidad</label>
										<div class="col-sm-9">
											<input type="text" class="form-control" id="inputCuota" name="inputCuota" required>
										</div>
									</div>
									<div class="form-group">
										<label for="inputTypeService" class="col-sm-3 control-label">Tipo de servicio</label>
										<div class="col-sm-9">
											<select class="form-control" id="inputTypeService" name="inputTypeService" required></select>
										</div>
									</div>
								</div><!-- ./modal-body -->
			...
			<!-- modal editar comunidad -->
				<form class="form-horizontal" id="formUpdCuota" name="formUpdCuota">
					<div class="modal fade" id="modalUpdCuota" tabindex="-1" role="dialog" aria-labellebdy="myModalLabel">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
								...
								<div class="modal-body">
									<div class="form-group">
										<label for="inputYear" class="col-sm-3 control-label">Año</label>
										<div class="col-sm-9">
											<input type="hidden" id="inputIDCuota" name="inputIDCuota" >
											<input type="hidden" id="inputIDColony" name="inputIDColony" value="<?= $idColony; ?>">
											<select class="form-control" id="inputYear" name="inputYear" required>
												<?php 
													$i = 1980;
				...
				<div class="col-md-12">
					<div class="box">
						<div class="box-header with-border">
							<h3 class="box-title">Cuotas de: <span class="nameColony"></span></h3>
						...
						<div class="box-body">
							<div class="table table-condensed table-hover table-striped">
								<table class="table table-striped table-bordered" id="data">
									<thead>
										<tr>
											<th><span title="servicioNombre">Tipo de Servicio</span></th>
  ...
  <!-- /.content-wrapper -->
	...
	<script>
		...
		$(document).ready(function(){
			//Obtenemos nombre de la comunidad
			$.ajax({
				type: "POST",
				...
				success: function(msg){
					...
					$(".content .box-header .nameColony").html(datos.dataRes[0].nombre);
					$(".loader").hide();
				},
				error: function(x, e){
					var cadErr = '';
			...
			//Obtenemos los tipos de servicios
			$.ajax({
				type: "POST",
				...
				success: function(msg){
					...
					if(msg.error == 0){
						$("#formAddCuota .modal-body #inputTypeService").html("");
						$.each(msg.dataRes, function(i, item){
							$("#formAddCuota .modal-body #inputTypeService").append($('<option>', { 
								value: msg.dataRes[i].id,
								text : msg.dataRes[i].nombre
							}));
							$("#formUpdCuota .modal-body #inputTypeService").append($('<option>', { 
								value: msg.dataRes[i].id,
								text : msg.dataRes[i].nombre
							}));
						});
			...
			function filtrar(){
				...
				$.ajax({
					type: "POST",
					data: {ordenar: ordenar, idColony: <?=$idColony;?>},
					url: "../controllers/get_cuotas.php",
					success: function(msg){
						console.log(msg);
						var msg = jQuery.parseJSON(msg);
						if(msg.error == 0){
							...
							$.each(msg.dataRes, function(i, item){
								var newRow = '<tr>'
									+'<td>'+msg.dataRes[i].nombreServicio+'</td>'
									...
									+'<ul class="dropdown-menu">'
										+'<li><a href="#" data-toggle="modal" data-target="#modalUpdCuota" id="editar" data-value="'+msg.dataRes[i].id+'"><i class="fa fa-edit"></i> Editar</a></li>'
										+'<li><a href="#" id="borrar" data-value="'+msg.dataRes[i].id+'" ><i class="fa fa-trash"></i> Eliminar</a></li>'
									+'</ul></div></td>'
									+'</tr>';
			...
			$("#data").on("click", "#editar", function(){
				var idCuota = $(this).data('value');
				console.log(idCuota);
				$(".loader").show();
				$.ajax({
					type: "POST",
					url: "../controllers/get_cuotas.php",
					data: {idCuota: idCuota, idColony: <?=$idColony;?> },
					success: function(msg){
						console.log(msg);
						var datos = jQuery.parseJSON(msg);
						$("#modalUpdCuota .modal-body #inputIDCuota").val(datos.dataRes[0].id);
						$("#modalUpdCuota .modal-body #inputCuota").val(datos.dataRes[0].cant);
						$("#modalUpdCuota .modal-body #inputYear").empty();
						//Obtenemos el select año
						var idYear = parseInt(datos.dataRes[0].year);
						for(var a = 2018; a >= 1980; a--){
							if(idYear == a){
								$("#modalUpdCuota .modal-body #inputYear").append($('<option>', { 
									value: a,
									text : a,
									selected: true
								}));
						...
						var typeService = datos.dataRes[0].nombreServicio;
						$.ajax({
							type: "POST",
							url: "../controllers/get_services.php",
							success: function(msg){
								...
								var msg = jQuery.parseJSON(msg);
								if(msg.error == 0){
									$("#modalUpdCuota .modal-body #inputTypeService").html("");
									$.each(msg.dataRes, function(i, item){
										if(typeService == msg.dataRes[i].nombre){
											$("#modalUpdCuota .modal-body #inputTypeService").append($('<option>', { 
												value: msg.dataRes[i].id,
												text : msg.dataRes[i].nombre,
												selected: true
											}));
        ...
        $(".modal-body .tab-content "+tab).addClass("active");
			...
			$("#data").on("click", "#borrar", function(){
				var idCuota = $(this).data('value');
				var msgBorrar = '¿Seguro que deseas eliminar ésta cuota?';
				if(confirm(msgBorrar)){
					$(".loader").show();
					$.ajax({
						type: "POST",
						url: "../controllers/delete_cuota.php",
	...
	<script>
		var form1 = $( "#formAddCuota" );
		form1.validate({ 
			ignore: "",
			rules:{
				inputYear: {required: true},
				inputCuota: {required: true, number: true},
				inputTypeService: {required: true}
			},
			messages:{
				inputYear: "Año obligatorio",
				inputCuota: {
					required: "Cuota obligatoria",
					number: "Solo se permiten números"
				},
				inputTypeService: "Tipo de servicio obligatorio",
			},
		...
		$( "#formAddCuota" ).submit(function( event ) {
			...
			if (form1) {
				...
				$.ajax({
					type: "POST",
					url: "../controllers/create_cuota.php",
					data: parametros,
					success: function(msg){
						var datos = jQuery.parseJSON(msg);
						if(datos.error == 0){
	...
	<script>
		...
		$( "#formUpdCuota" ).submit(function( event ) {
			...
			if (form2) {
				...
				$.ajax({
					type: "POST",
					url: "../controllers/update_cuota.php",
					data: parametros,
					success: function(msg){
						console.log(msg);
						var datos = jQuery.parseJSON(msg);
